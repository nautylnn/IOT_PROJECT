...
String mainPage()
{
  ...
  <head>
    ...
    background: rgba(11, 19, 40, 0.35); /* màu hòa với nền canvas */
  ...
  .container1 {   
    ...
    z-index: 200;         /* <<< BẮT BUỘC để không bị top-bar che */
    margin-top: -150px;   
  }
  ...
  #bgCanvas {
    ...
    height: 100vh;
    z-index: -1;
    background: radial-gradient(circle at 30% 50%, #0f1a2d, #0a0f24 60%, #05080f 100%);
  }

  /* Màu chữ trắng để nổi trên nền */
  body {
    color: white;
  }

  .right-controls {
    margin-top: -30px;   /* đẩy riêng cụm icon lên */

    display: flex;
    align-items: center;
    gap: 5px;  /* khoảng cách giữa icon */
  }
...
/* BUTTONS */
  ...
  <div class="top-bar">
    ...
    <div class="right-controls">
      <button class="icon-btn" id="notif" onclick="alert('No new notifications')">🔔</button>
      <button class="icon-btn" id="settings" onclick="window.location='/settings'">⚙️</button>
    </div>
  </div>

<div class="sensor-row-all">
...
<div class="sensor-row-all">
    <div class="sensor-big">
      ...
      <span class="sensor-title">Sensors</span>
    </div>

    <div class="sensor-box">
        <div class="sensor-box-title">🌡️</div>
        <div class="sensor-box-value">
             <span id="temp">)rawliteral" + String(temperature) + R"rawliteral(</span> °C
             
        </div>
    </div>

    <div class="sensor-box">
        <div class="sensor-box-title">💧</div>
        <div class="sensor-box-value">
...
<div class="sensor-row-all">
    ...
    <div class="sensor-big">
      ...
      <span class="sensor-title">Machine Learning</span>
    </div>

    <div class="sensor-box">
        <div class="sensor-box-title">👽</div>
        <div class="sensor-box-value">
         AI prediction: <span id="result">)rawliteral" + result + R"rawliteral(</span> 
    ...
    <div class="sensor-box">
        <div class="sensor-box-title">🚨</div>
        <div class="sensor-box-value">
          Data anomaly rate: <span id="percent">)rawliteral" + String(percent) + R"rawliteral(</span> % 
  ...
  <div class="led-container">
    ...
    <div class="led-box">
      ...
      <div class="led-buttons">
        <button class="led-btn enable-btn" onclick="toggleLED(1)">Enable</button>
        <button class="led-btn disable-btn" onclick="toggleLED(1)">Disable</button>
      </div>
    </div>
    ...
    <div class="led-box">
      ...
      <div class="led-buttons">
        <button class="led-btn enable-btn" onclick="toggleLED(2)">Enable</button>
        <button class="led-btn disable-btn" onclick="toggleLED(2)">Disable</button>
      </div>
    </div>
...
function animate() {
  // Nền gradient nhẹ giống màu trong hình
  ...
  for (let x = 0; x < canvas.width; x++) {
    let y = canvas.height/2 +
            Math.sin((x + t) * 0.008) * 40 +
            Math.sin((x + t*0.7) * 0.004) * 30;
    ctx.lineTo(x, y);
  }
  ctx.stroke();
  ctx.closePath();

  // Update và vẽ hạt sáng
  particles.forEach(p => {
    ctx.beginPath();
...
String settingsPage()
{
  ...
  <head>
    ...
    padding: 6px 20px;
    display: flex;
    justify-content: space-between;  /* <<< CHỈNH QUAN TRỌNG */
    align-items: center;
...
// ========== Handlers ==========
void handleRoot() { serverAP.send(200, "text/html", mainPage()); }

void handleToggle()
{
  int led = serverAP.arg("led").toInt();
  if (led == 1)
  {
    led1_state = !led1_state;
    digitalWrite(47, led1_state);
  ...
  else if (led == 2)
  {
    ...
    digitalWrite(38, led2_state);
    Serial.println("State led2: ");
    Serial.println(led2_state ? "ON" : "OFF");
  }
  serverAP.send(200, "application/json",
...
void handleSettings() { serverAP.send(200, "text/html", settingsPage()); }

void handleConnect()
{
  wifi_ssid = serverAP.arg("ssid");
  wifi_password = serverAP.arg("pass");
  serverAP.send(200, "text/plain", "Connecting....");
  isAPMode = false;
  connecting = true;
...
void setupServer()
{
  ...
  serverAP.on("/toggle", HTTP_GET, handleToggle);
  serverAP.on("/sensors", HTTP_GET, handleSensors);
  serverAP.on("/settings", HTTP_GET, handleSettings);
  serverAP.on("/connect", HTTP_GET, handleConnect);
  serverAP.begin();
}
